defmodule MydiaWeb.CoreComponents do
  ...
  Owns that corner so nothing else claims `absolute top-2 right-2`. Renders
  nothing when no badge applies, so no empty positioned container is emitted.

  ## Examples

      <.poster_badges id="poster-badges-1" quality="1080p" />
  """
  ...
  def poster_badges(assigns) do
    ...
    >
      <span class="badge badge-neutral badge-sm shadow-md">{@quality}</span>
    </div>
    """
  end
